...
class NetworkXStorage(BaseGraphStorage):
    ...
    async def get_edge_with_content(
        ...
        """获取边数据并动态生成content字段"""
        edge_data = dict(self._graph.edges.get((source_node_id, target_node_id), {}))
        if edge_data:
            # 动态生成content字段
            ...
            if relation_name != "":
                ...
                edge_data["content"] = "{keywords} {src_id} {tgt_id} {description}".format(
                    keywords=keywords, 
                    src_id=edge_data.get("src_id", source_node_id), 
                    tgt_id=edge_data.get("tgt_id", target_node_id),
