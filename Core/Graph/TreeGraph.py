...
class TreeGraph(BaseGraph):
    ...
    async def _perform_clustering(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
            for batch_start in range(0, n_global_clusters, batch_size):
                batch_end = min(batch_start + batch_size, n_global_clusters)
                cluster_tasks = [pool.submit(self._create_task_with_return(self._process_cluster),
                                          i=i,
                                          global_clusters=global_clusters,
                                          embeddings=embeddings,
                                          dim=dim,
                                          threshold=threshold) for i in range(batch_start, batch_end)]
                
                # 等待当前批次完成，然后处理结果
                for task in as_completed(cluster_tasks):
                ...
                logger.info(f"Processing clustering batch {batch_start}-{batch_end-1} of {n_global_clusters} clusters")
                time.sleep(4)
        ...
        for layer in range(self.config.num_layers):  # build a new layer
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
                for batch_start in range(0, len(clusters), batch_size):
                    ...
                    cluster_tasks = [pool.submit(self._create_task_for(self._extract_cluster_relationship_without_embedding), layer = layer + 1, cluster = clusters[j]) for j in range(batch_start, batch_end)]
                    # 等待当前批次的任务完成
                    for task in as_completed(cluster_tasks):
                        task.result()  # 确保任务完成
                    logger.info(f"Processing cluster relationship batch {batch_start}-{batch_end-1} of {len(clusters)} clusters")
                    time.sleep(3)
        ...
        if self.config.build_tree_from_leaves:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
                ...
                for batch_start in range(0, len(chunks), batch_size):
                    ...
                    leaf_tasks = [pool.submit(self._create_task_for(self._extract_entity_relationship_without_embedding), chunk_key_pair=chunks[index]) for index in range(batch_start, batch_end)]
                    # 等待当前批次的任务完成
                    for task in as_completed(leaf_tasks):
                        task.result()  # 确保任务完成
                    logger.info(f"Processing leaf nodes batch {batch_start}-{batch_end-1} of {len(chunks)} chunks")
                    # time.sleep(2)
