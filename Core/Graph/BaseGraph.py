...
class BaseGraph(ABC):
    ...
    async def augment_graph_by_similarity_search(self, entity_vdb, duplicate=False):
        ...
        for edge in await self._graph.edges():
            edge_data = await self._graph.get_edge(edge[0], edge[1])
            if edge_data and edge_data.get("relation_name") == "similarity":
                similarity_edge_count += 1
        
        if similarity_edge_count > 0:
            logger.info(f"Found {similarity_edge_count} existing similarity edges. Skipping graph augmentation.")
            return
