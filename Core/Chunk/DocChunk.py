...
class DocChunk:
    ...
    async def build_chunks(self, docs: Union[str, List[str]], force=False):
        ...
        if not is_exist or force:
            if is_exist and force:
                logger.info("强制重新执行chunk过程")
            elif not is_exist:
                logger.info("未找到缓存数据，开始执行chunk过程")
    ...
    async def get_chunk_by_index(self, index):
        """获取完整的TextChunk对象，而不只是content"""
        chunk = await self._chunk.get_data_by_index(index)
        return chunk
    
    async def get_chunks_by_indices(self, indices):
        """获取完整的TextChunk对象列表，而不只是content"""
        return await asyncio.gather(
            *[self.get_chunk_by_index(index) for index in indices]
        )
