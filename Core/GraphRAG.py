...
class GraphRAG(ContextMixin, BaseModel):
    ...
    async def _build_retriever_context(self):
        ...
        try:
            for context_name, use_context in self._retriever_context.items():
                if use_context:
                    # 特殊处理映射对象：它们是类属性而不是实例属性
                    if context_name in ["relationships_to_chunks", "entities_to_relationships"]:
                        config_value = getattr(self.__class__, context_name, None)
                        if config_value is None:
                            logger.warning(f"映射对象 {context_name} 未在类中找到，可能未初始化")
                            continue
                        else:
                            logger.info(f"✅ 成功从类属性获取映射对象 {context_name}: {type(config_value)}")
                    else:
                        config_value = getattr(self, context_name)
                        
                    if context_name == "config":
                        # 注册完整的Config对象以便访问retrieval_llm等配置
                        self.retriever_context.register_context("full_config", self.config)
                        # 同时保持原有的retriever config注册
    ...
    # haloayng 智能检查是否需要重建E2R-R2C映射
    async def _check_if_e2r_r2c_needs_rebuild(self):
        """
        智能检查是否需要重建E2R-R2C映射
        
        返回:
            bool: True表示需要重建，False表示可以跳过
        """
        try:
            # 检查映射文件是否存在
            e2r_file = self.entities_to_relationships.namespace.get_load_path(self.entities_to_relationships.RESOURCE_NAME)
            r2c_file = self.relationships_to_chunks.namespace.get_load_path(self.relationships_to_chunks.RESOURCE_NAME)
            
            if not e2r_file or not r2c_file:
                logger.info("E2R-R2C映射文件不存在，需要重建")
                return True
            
            import os
            if not os.path.exists(e2r_file) or not os.path.exists(r2c_file):
                logger.info("E2R-R2C映射文件不存在，需要重建")
                return True
            
            # 尝试加载现有映射以检查其结构
            current_edge_count = self.graph.edge_num
            
            # 简单检查：如果映射文件存在且图边数没有大幅变化，则跳过重建
            # 这是一个保守的策略，确保映射与当前图结构匹配
            try:
                await self.entities_to_relationships.load(False)
                current_mapping = await self.entities_to_relationships.get()
                
                if current_mapping is not None:
                    # 检查映射的边数是否与当前图匹配（允许小幅差异）
                    mapping_edge_count = current_mapping.shape[1] if hasattr(current_mapping, 'shape') else 0
                    edge_count_diff = abs(current_edge_count - mapping_edge_count)
                    
                    if edge_count_diff == 0:
                        logger.info(f"E2R-R2C映射已存在且边数匹配 ({current_edge_count} 条边)，跳过重建")
                        return False
                    else:
                        logger.info(f"图边数发生变化 ({mapping_edge_count} -> {current_edge_count})，需要重建映射")
                        return True
                else:
                    logger.info("无法加载现有E2R映射，需要重建")
                    return True
            except Exception as load_error:
                logger.info(f"加载现有映射时出错: {load_error}，需要重建")
                return True
                
        except Exception as e:
            logger.info(f"检查E2R-R2C映射状态时出错: {e}，为安全起见重建映射")
            return True
        
    def _update_costs_info(self, stage_str:str):
        last_cost = self.llm.get_last_stage_cost()
        display_model_name = last_cost.model_name if last_cost.model_name else f"{self.llm.config.model}(未调用)"
        logger.info(f"{stage_str} stage cost: 模型: {display_model_name} | Total prompt token: {last_cost.total_prompt_tokens}, Total completeion token: {last_cost.total_completion_tokens}, Total cost: {last_cost.total_cost}")
    ...
    async def insert(self, docs: Union[str, list[Any]]):
        ...
        if self.config.use_entity_link_chunk:
            # haloayng 智能检查是否需要重建E2R-R2C映射
            need_rebuild = await self._check_if_e2r_r2c_needs_rebuild()
            await self.build_e2r_r2c_maps(need_rebuild)
